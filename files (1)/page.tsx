...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([fetch("/api/customers"), fetch("/api/machines")]).then(
      async ([cRes, mRes]) => {
        if (cRes.status === 401) {
          router.push("/login");
          return;
    ...
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <section className="mt-10">
        ...
        <p className="text-sm text-vilike-muted">
          Demo MSN tracking: {DEMO_MSN_TRACKING}
        </p>
        <div className="mt-4">
      ...
      <section className="mt-10">
        ...
        <p className="mb-4 text-sm text-vilike-muted">
          Edit model name, price, and hero image URL
        </p>
        <AdminMachineManager initialMachines={machines} />
      </section>

      {/* ── NEW: Create customer login ── */}
      <section className="mt-10">
        <AdminCreateCustomer />
      </section>
    </main>
